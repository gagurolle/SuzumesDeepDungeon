...
namespace SuzumesDeepDungeon.Services
{
    public class SteamApi
    {
        ...
        public static string rawgApi { get; set; } = "";
        public SteamApi(ILogger<SteamApi> logger)
        {
            _logger = logger;
        }

        public async Task<string> GetGameJpg(string steamUrl)
        {
        ...
        /// <summary>
        /// get game cover image from steam
        /// </summary>
        /// <param name="steamAppId"></param>
        /// <param name="url"></param>
        /// <returns></returns>
        public async Task<string?> GetGameCoverAsync(string steamAppId, string url = SteamImageBaseUrl_600x900)
        {
        ...
        /// <param name="url"></param>
        /// <returns></returns>
        public string GetSteamAppIdFromUrl(string url)
        {
    ...
    public class RawgApi
    {
        ...
        public static string rawgApi { get; set; } = "";
        public RawgApi(ILogger<RawgApi> logger, SteamApi steamApi)
        {
            _logger = logger;
            _steamApi = steamApi;
        }
        public async Task<RawgGameCompleteData> GetGameFullInfo(string? gameName = null, string? id = null)
        {
        ...
        public async Task<GameRank> RawgDataToGameRank(RawgGameCompleteData data, User user = null)
        {
            ...
            if (data?.steamUrl != null)
            {
                try
                {
                    var steamIdFromurl = _steamApi.GetSteamAppIdFromUrl(data?.steamUrl);

                    gameRank.Screenshots = new Screenshot()
                    {
                        SteamHeaderUrl = await _steamApi.GetGameCoverAsync(steamIdFromurl, SteamApi.SteamImageBaseUrl_header) ?? "",
                ...
                {
                    _logger.LogError($"Error getting steam cover: {ex.Message}");
        ...
        public GameVideosResponse GetGameTrailers(string id)
        {
        ...
        public StoreApiResponse GetGameStores(string id)
        {
            using (var client = new HttpClient())
            {
        ...
        public AchievementsApiResponse GetGameAchievements(string id, int page = 1)
        {
        ...
        public async Task<List<Game>> FindGames(string gameName)
        {
